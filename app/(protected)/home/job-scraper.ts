...
const EDGE_FN_URL = `${SUPABASE_URL}/functions/v1/scraping-progress`;
...
type RemoteJob = {
    ...
    company_metadata: any | null;
};

async function createRemoteJob(targetUrl: string, initialSteps: ScraperStep[]) {
    const res = await fetch(EDGE_FN_URL, {
        ...
        body: JSON.stringify({ url: targetUrl, initialSteps }),
    });
    ...
    const job: RemoteJob = await res.json();
    return job;
}

async function patchRemoteJob(jobId: string, payload: Partial<RemoteJob>) {
    const res = await fetch(`${EDGE_FN_URL}?jobId=${jobId}`, {
        method: "PATCH",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (!res.ok) {
        console.warn("Failed to update remote scraping job");
        return null;
    }
...
async function getRemoteJob(jobId: string) {
    const res = await fetch(`${EDGE_FN_URL}?jobId=${jobId}`, {
        method: "GET",
        headers: {
            Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (!res.ok) {
        throw new Error("Failed to fetch scraping job");
    }
...
export function useJobScraper() {
    ...
    // keep jobId in localStorage so refresh can recover
    useEffect(() => {
        const stored = typeof window !== "undefined"
            ? window.localStorage.getItem("scraping_job_id")
            : null;
        if (stored && !jobId) {
            setJobId(stored);
        }
    ...
    useEffect(() => {
        if (jobId && typeof window !== "undefined") {
            window.localStorage.setItem("scraping_job_id", jobId);
        }
    }, [jobId]);

    const updateProgressRemote = async (updates: Partial<RemoteJob>) => {
        if (!jobId) return;
        await patchRemoteJob(jobId, updates);
    };
    ...
    const restoreProgress = async (existingJobId?: string) => {
        const idToUse = existingJobId ?? jobId;
        if (!idToUse) return null;
        try {
            const job = await getRemoteJob(idToUse);
            setJobId(job.id);
            if (job.steps?.length) setSteps(job.steps);
            setLoading(job.status === "in-progress");
            if (job.error_message) setError(job.error_message);
            return job;
        } catch (err) {
    ...
    const startScrape = async (rawUrl: string, isRecoveryAttempt = false) => {
        ...
        // if new run, create a remote job
        if (!isRecoveryAttempt) {
            try {
                const remote = await createRemoteJob(
                    normalizedUrl,
                    INITIAL_STEPS,
                );
                setJobId(remote.id);
                ...
                setLoading(false);
                setError(e.message ?? "Failed to initialize job");
                return false;
            }
        }
        ...
        try {
            ...
            await updateProgressRemote({ status: "in-progress" });
            ...
            if (finalJobLinks.length > 0 || isRecoveryAttempt) {
                try {
                    ...
                    if (crawlRes.ok) {
                        ...
                        const crawledLinks = (crawlData.data || [])
                            .map((d: any) => d.url)
                            .filter(Boolean);
            ...
            // incremental scraping / existing URLs
            ...
            await updateProgressRemote({
                jobs_found: finalJobLinks.length,
            });
            ...
            await updateProgressRemote({
                jobs_found: deduplicatedJobsList.length,
            });

            // ---------------- STEP 4 ----------------
            if (deduplicatedJobsList.length > 0) {
                updateStep("4", { status: "in-progress" });
                const saveRes = await saveJobsToSupabase(
                    deduplicatedJobsList,
                    {
                        replaceCompanyJobs: false,
            ...
            await updateProgressRemote({ status: "completed" });
            return true;
        } catch (err: any) {
            console.error(err);
